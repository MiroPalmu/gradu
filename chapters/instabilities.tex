...
whose unstable modes are fully electromagnetic~\citep{bret_collective_2004}.
...
Beam-plasma systems have been studied since \citet{langmuir_scattering_1925}
first observed the existence of oscillations in such systems~\citep{briggs_electron-stream_1964}.
Later, \citet{pierce_possible_1948} explained Langmuir's observations by demonstrating
that unstable oscillations can arise in such systems.
This prompted Bohm and Gross to develop the kinetic theory of ''two-stream instability'',
which contains unstable waves propagating along the beam direction \citep{bohm_theory_1949}.
\citet{fried_mechanism_1959} showed that a second kind of instability exists,
...
Fried's article also mentioned closely related work by~\citet{weibel_spontaneously_1959},
which demonstrated the instability of an anisotropic, two-temperature Maxwellian plasma.
In literature, these two instabilities have become almost interchangeable,
even though they are technically not equivalent~\citep{bret_multidimensional_2010}.
...
This was promptly done in the cold-fluid limit, where it was found that
the unstable spectrum is two-dimensional,
as the oblique perturbations are likely to be unstable
\citep{watson_statistical_1960,bludman_statistical_1960,fainberg_nonlinear_1970}.
...
\citep{bret_collective_2004,bret_electromagnetic_2005,bret_characterization_2005},
supported by numerous PIC simulations~\citep{gremillet_linear_2007,
...
Following the symbolic-calculator technique from~\citet{bret_beam-plasma_2007},
we solve for $\ve V_{b1}$ and $\ve V_{p1}$ from the linearized momentum transport equation
(see Equation~\ref{eq:linearized-cold-fluid-momentum-transport})
and insert them to the linearized continuity equation
(see Equation~\ref{eq:linearized-cold-fluid-continuity-eq})
to obtain $n_{b1}$ and $n_{p1}$.
These are then inserted into the linearized wave equation
(see Equation~\ref{eq:linearized-wave-eq}).
The resulting expression can be rearranged into
a matrix form~\citep{bret_collective_2004,bret_cfa_2012}:
\begin{align}
...
\begin{figure}
  ...
  \caption{Growth rates from dispersion relation seen in Equation~\eqref{eq:dispersion-eq}
    for $\gamma_b = 3$.
...
$\boldsymbol{\epsilon}(\ve k, \omega)$ simplify to~\citep{bret_cfa_2012}\footnote{
  Note that in \citep{bret_cfa_2012} powers of $-3$ are missing from Equation~\eqref{eq:mistake-a}
...
\end{align}
Inserting these into the dispersion relation (see Equation~\ref{eq:dispersion-eq})
...
\begin{align}
  ...
  \Im (x) &= \frac{V_b}{c}\sqrt{\frac{2}{\gamma_b}}, \quad \alpha = 1
\end{align}

\section{Numerical validation}

GPU-accelerated Runko was used to simulate filamentation instability
of symmetric beams ($\alpha = 1$) following the setup from~\citep{nattila_runko:_2022}.
The two counter-streaming pair-plasma populations were initialized to
...
The plasmas were modeled with 32 particles per cell per species,
which initial positions were sampled from a unifrom distribution
and velocities were sampled from Jüttner-Synge distribution~\citep{zenitani_loading_2015}
...
Initially, the electromagnetic fields were set to zero.
The periodic simulation domain consisted of a grid of $320\times80\times6$ uniform cells
and the skin depth was resolved with 10 cells.
The time step was related to the cell width as: $\Delta t = 0.45 \Delta x$.
...
Figure~\ref{fig:num-growth-rates} shows the average magnetic-field energy density,
