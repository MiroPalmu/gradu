...
\section{Linear analysis in the cold-fluid limit}

As stated at the beginning of the chapter our system consists of
two homogeneous counter-streaming pair plasma populations.
Ions are concidered as a fixed neutralizing background.
We'll call the two populations the beam and the background plasma
and denote their number densities as $n_b$ and $n_p$.
Similarly we'll denote their bulk velocities as $\mathbf{V}_b$ and $\mathbf{V}_p$.
Initially there are no electromagnetic fields.

We will linearize 


and corresponding Lorentz factors $\gamma_b$ and $\gamma_p$ respectively.
and the system is initially charge and current neutral:

\begin{align}
  \label{eq:beam-neutrality}
  n_b + n_p = n_i \\
...
The system is assumed j


We'll investigate a homogeneous and infinite beam-plasma system is
composed of a beam and a background plasma.
Denoting the number density of the beam with $n_b$ and background with $n_p$,




The cold-fluild is characterized with distribution function $f_\alpha (\mathbf{x}, \mathbf{p}, t)$
