...
the existence of oscillations in beam-blasma
...
The resulting equation can be manipulated to a matrix form \cite{bret_collective_2004,bret_cfa_2012}:
\begin{align}
...
$\boldsymbol{\epsilon}(\ve k, \omega)$ simplify to \cite{bret_cfa_2012}\footnote{
  ...
  and there is extra $\gamma_p^{-1}$ in \eqref{eq:mistake-b}.
}:
\begin{align}
...
\begin{figure}
  ...
  \caption{Simulation of the filamentation instability using the GPU port of Runko
    with different values of $\gamma_b$. Solid curves plot the average value of
    ...
    as a function on time $t\omega_p$.
