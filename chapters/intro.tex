...
Plasma dynamics has been studied in many contexts, including astrophysics, laser physics, and fusion~\citep{uzdensky_plasma_2014,umstadter_relativistic_2003,kirkwood_review_2013}.
...
One commonly used approach is the particle-in-cell (PIC) method,
in which the electromagnetic fields are discretized on a grid
while the particles are free to move continuously in space.

This thesis is based on the work carried out by the author to port
the PIC simulation framework Runko~\citep{nattila_runko:_2022} to GPUs.
The Git repository of Runko is located at
\href{https://github.com/hel-astro-lab/runko}{github.com/hel-astro-lab/runko}.
At the time of writing this thesis,
the commits related to the GPU port reside on the \texttt{dev-v5} branch.
All later references to file locations inside the Runko repository
specifically refer to the repository state at the Git commit
with the abbreviated hash \texttt{92d840e}\footnote{
  The full hash is \texttt{92d840eb20cba166d394d3b256d50b7335e984e9}.
}.

In this chapter, the required background theory is presented,
and in the following chapter, different aspects of PIC simulation are discussed.
In the third chapter, the GPU port of Runko is used to simulate
the electron beam-plasma filamentation instability,
which is then compared to the analytical result
in order to confirm physical accuracy of the port.
The final chapter contains conclusions and
a discussion of further development avenues for the GPU port of Runko.
...
over interactions between individual particles~\citep{pert_foundations_2021,hazeltine_framework_2018}.
...
\end{align}
where the sum is taken over all particle species species $s$, $n_s$ denotes the number density,
and $q_s$ is the charge.
...
the scale of number density deviations from quasi-neutrality
over a length $L$ due to thermal fluctuations is
$\left(\lambda_D/L\right)^2$~\citep{wiesemann_short_2014,hazeltine_framework_2018}.
...
the ''Debye sphere'': $N_D = \frac {4\pi}{3}n \lambda_D^3 \gg 1$.
...
and written as~\citep{kilpula_2017}:
\begin{align}
...
known as the plasma frequency~\citep{wiesemann_short_2014,pert_foundations_2021,hazeltine_framework_2018}.
For a particle species $s$, the classical plasma frequency is given by:
\begin{align}
  \label{eq:classical-plasma-frequency}
  \omega_{\mathrm{p}s} = \sqrt{\frac{n_s q_s^2}{\epsilon_0 m_s}}\,.
\end{align}
...
also known as Larmor frequency.
For a particle species $s$, the cyclotron frequency is given by:
\begin{align}
...
As seen from the condition in Equation~\eqref{eq:plasma-param-condition},
the number of particles in plasma is large.
...
$[\ve p, \ve p + d\ve p]$~\citep{klimontovich_relativistic_1960,DeGroot:1980dk,hazeltine_framework_2018}.
...
Due to the condition in Equation~\eqref{eq:plasma-param-condition},
short-range collisions between particles can be neglected, and the collision term vanishes.
When the collision term vanishes, the equation is called the Vlasov equation:
\begin{align}
  \label{eq:vlasov}
  \partial_t f_s
  + \mathbf{v} \cdot \nabla f_s
  + q_s ( \mathbf{E} + \boldsymbol{\beta} \times \mathbf{B}) \cdot \nabla_\mathbf{p} f_s = 0\,,
\end{align}
...
that describes the dynamics of plasma.

\section{Cold-fluid limit}

Solving the Vlasov-Maxwell system is challenging due to the non-linear nature of plasma dynamics.
...
In general, the moment $M(\mathbf{p})$ of the Vlasov equation (see Equation~\ref{eq:vlasov}) is:
\begin{align}
\int M \left(
\partial_t f_s
+ \mathbf{v} \cdot \nabla f_n
...
For brevity, we define $a_s^i = q_s E^i + c^{-1}q_s\epsilon^i_{lk}v^l B^k$,
where $\epsilon^i_{lk}$ is the totally antisymmetric Levi-Civita symbol
and Einstein summation convention is followed.
Using the identity:
\begin{align}
\partial_{p_i} v^l = \frac{\delta^l_i}{m \gamma} -  \frac{p^lp_i}{m^3c^2\gamma^3}\,,
\end{align}
where $\delta^i_j$ is the Kronecker delta, we find that $\partial_{p_i}a_s^i = 0$.
After integration by parts and noting
that physical distributions vanishes as $\ve p$ approaches infinity,
we can write the third term as:
\begin{align}
\int M \mathbf{a_s} \cdot \nabla_\mathbf{p} f_s d\mathbf{p}
&= - \int a^i \partial_{p_i} M f d\mathbf{p}\,.
\end{align}
We obtain:
\begin{align}\label{eq:general-moment-of-vlasov}
  \partial_t \int M f_s d\mathbf{p}
+ \partial_i \int M v^i f_s d\mathbf{p}
= \int a^i \partial_{p_i} M f_s d\mathbf{p}\,.
\end{align}

A cold-fluid is characterized by a distribution function
$f(\mathbf{x}, \mathbf{p}, t) = n_s(\ve x, t)\delta(\mathbf{p} - \mathbf{P}_s(\mathbf{x}, t))$,
where $\mathbf{P}_s(\mathbf{x}, t) = m_s\gamma_s\mathbf{V}_s(\mathbf{x}, t)$
is calculated from the bulk velocity $\mathbf{V}_s(\mathbf{x}, t)$ of the fluid,
and $n_s(\mathbf{x}, t)$
is its number density~\citep{bret_multidimensional_2010,bret_collective_2004}.
Inserting this and $M = 1$ into Equation~\eqref{eq:general-moment-of-vlasov},
we obtain the continuity equation~\citep{hazeltine_framework_2018}:
\begin{align}
\label{eq:continuity-in-cold-limit}
\partial_t n_s + \nabla \cdot ( \ve V_s n_s ) = 0\,.
\end{align}
Using the continuity equation for $M = \mathbf{p}$, we obtain:
\begin{align}
\label{eq:momentum-transport-in-cold-limit}
\partial_t \mathbf{P}_s + (\mathbf{V}_s \cdot \nabla) \mathbf{P}_s
&= q_s (\mathbf{E} + c^{-1}\mathbf{V}_s \times \mathbf{B})\,.
\end{align}
...
In this thesis, linear-order solutions to Equations~\eqref{eq:continuity-in-cold-limit}
and~\eqref{eq:momentum-transport-in-cold-limit}
...
we can write (omitting particle species labels for brevity):
\begin{align}
...
\end{align}
where we defined $\gamma \equiv \gamma[\ve V_0]$.

Inserting these into Equations~\eqref{eq:continuity-in-cold-limit}
and~\eqref{eq:momentum-transport-in-cold-limit},
we obtain the linearized continuity
and the linearized momentum transport equations~\citep{bret_collective_2004,bret_cfa_2012}:
\begin{align}
  ...
  \left(\hat{\ve V}_1 + \frac{\gamma^2}{c^2}(\ve V_0 \cdot \hat{\ve V}_1)\ve V_0\right)
  &= q\left(\hat{\ve E}_1
    + \frac 1c \hat{\ve V}_1 \times \ve B_0 + \frac 1c \ve V_0 \times \hat{\ve B}_1 \right)\,.
\end{align}
Additionally, we utilize linearity of Maxwell's equations (see Equation~\ref{eq:maxwells})
...
\end{align}
where $\hat{\ve J}_1$ in the cold-fluid limit is:
\begin{align}
  \hat{\ve J}_1 &=\sum_s q_s(n_{s0}\hat{\ve V}_{s1} + \hat{n}_{s1}\ve V_{s0})\,.
\end{align}
...
Let the function $f(x)$ be discretized into uniform grid of $n$ points with a separation $h$,
...
\begin{align}
  ...
  \frac{f_{n+2} - 2 f_n + f_{n-2}}{4h^2}  = f''_n + O(h^2)\,.
\end{align}
We note that the central finite-difference second-order derivative in Equation~\eqref{eq:second-fd}
...
the constant coefficients in Equation~\eqref{eq:second-fd-accuracy}.

There exist higher-order accurate schemes for calculating the centered finite-difference,
but they involve a trade-off between improved numerical accuracy and increased computational cost.
...
its accuracy and stability properties.
The one-dimensional wave equation for function $\psi(t, x)$ is given by:
\begin{align}
  \label{eq:one-d-wave-eq}
  \frac{\partial^2 \psi}{\partial t^2} = c^2 \frac{\partial^2 \psi}{\partial x^2}\,.
\end{align}
Inserting the plane wave solution $\psi = e^{i(kx - \omega t)}$
to Equation~\eqref{eq:one-d-wave-eq} gives the physical dispersion relation:
\begin{align}
...
Applying the centered finite-difference for second derivatives
(see Equation~\ref{eq:second-fd-correct})
to both the temporal and spatial derivatives in Equation~\eqref{eq:one-d-wave-eq},
...
that is distinctly different from the physical dispersion relation
(see Equation~\ref{eq:one-d-wave-dispersion-eq}):
\begin{align}
...
or the Courant number for short~\citep{verboncoeur_particle_2005}.
The solutions are periodic in $k$ with a period of $\frac{2\pi}{\Delta x}$,
which means that the finite-difference wave equation in Equation~\eqref{eq:fd-intermediate}
cannot resolve waves beyond a certain wavenumber.

The numerical dispersion relation in Equation~\eqref{eq:fd-dispersion}
reduces to the physical dispersion relation from Equation~\eqref{eq:one-d-wave-dispersion-eq}
...
\begin{table}
  ...
  \begin{tabular}{c|c}
    ...
    $\hat{c} = 1$ & $\omega = \pm ck$ \\
    $\Delta t \rightarrow 0$ & $\omega \Delta x = \pm c \sqrt{2(1-\cos(k\Delta x))}$ \\
    $\Delta x \rightarrow 0$ & $\cos(\omega \Delta t) = -c^2k^2\frac{(\Delta t)^2}{2} + 1$ \\
    $\Delta t \rightarrow 0$ and $\Delta x \rightarrow 0$ & $\omega = \pm ck$
  \end{tabular}
  \caption{Summary of different limits for the numerical dispersion
    relation from Equation~\eqref{eq:fd-dispersion}~\citep{verboncoeur_particle_2005}.}
  \label{fig:numerical-dispersion-relation-limits}
\end{table}

The Courant number $\hat{c}$ is crucial for the stability of solutions to
the finite-difference wave equation in Equation~\eqref{eq:fd-intermediate}.
For $\hat{c} > 1$, the numerical dispersion relation in Equation~\eqref{eq:fd-dispersion}
shows that $\omega$ has a positive imaginary part for some values of $k$,
which results in an exponentially growing mode for $\psi(t, x)$.
The condition for stability, $\hat c = c\frac{\Delta t}{\Delta x} \le 1$,
is called the Courant-Friedrichs-Lewy condition.
In multiple dimensions, the condition is given by~\citep{verboncoeur_particle_2005}:
\begin{align}
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.7]{chapters/cfl-plot.pdf}
  \caption{One period of the numerical dispersion relation given in Equation~\eqref{eq:fd-dispersion}
    ...
    while the imaginary parts are shown as dashed curves.
    Courant numbers that satisfy the Courant-Friedrichs-Lewy condition have
    colors ranging from blue to green,
    whereas those that do not satisfy the condition have colors ranging from red to yellow.
    The black curve represents the transition point at $\hat c = 1$.
    For every plane wave solutions, there exists an another plane wave solutions
    propagating in the opposite direction.
    This manifests as symmetry along $\omega = 0$ in the numerical dispersion relation.
    The symmetry along $k\frac{\Delta x}{\pi} = 1$
    is a numerical aliasing artifact of the discrete grid,
    which cannot represent wavelengths $\lambda < 2\Delta x$.
  }
  \label{fig:cfl-plot}
\end{figure}
