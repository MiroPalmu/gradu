...
\begin{align}
  ...
  \omega_{\mathrm{p}s}^2 = \frac{n_s q_s^2}{\epsilon_0 m_s}\,.
\end{align}
...
In general, the moment $M(\mathbf{p})$ of the Vlasov equation (see Equation~\eqref{eq:vlasov}) is:
\begin{align}
\int M \left(
\partial_t f_s
+ \mathbf{v} \cdot \nabla f_n
...
\end{align}
Additionally, we utilize linearity of Maxwell's equations (see Equation~\eqref{eq:maxwells})
...
(see Equation~\eqref{eq:second-fd-correct})
...
(see Equation~\eqref{eq:one-d-wave-dispersion-eq}):
\begin{align}
...
which results in an exponentially growing mode for $\phi(t, x)$.
