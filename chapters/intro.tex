...
\begin{align}
  ...
  \int f_s(\ve x, \ve p, t) d\ve p = n_j(\ve x, t)
\end{align}
where $n_j(\ve x, t)$ is the number density of particle species $s$.
...
We want to linearize the equations \eqref{eq:continuity-in-cold-limit}
and \eqref{eq:momentum-transport-in-cold-limit}
in case where there is no initial electromagnetic fields.
In order to do so we'll denote small perturbations with subscript $1$,
so we can write:
\begin{align}
...
where $\gamma = \gamma(\ve V_0)$.
Inserting these and the inverse Fourier transforms
$f = \int \hat{f} \exp(i(\ve k \cdot \ve x - \omega t))d\omega d^3k$
of the perturbations
to equations \eqref{eq:continuity-in-cold-limit} and \eqref{eq:momentum-transport-in-cold-limit}
we get:
\begin{align}
  \label{eq:linearized-cold-fluid}
  \hat{n}_1 &= \frac{n_0 \ve k \cdot \hat{V}_1}{\omega - \ve k \cdot \ve V_0} \\
  im\gamma(\ve k \cdot \ve V_0 - \omega)
  (\hat{\ve V}_1 + \frac{\gamma^2}{c^2}(\ve V_0 \cdot \hat{\ve V_1})\ve V_0)
  &= q(\hat{E}_1 + \frac 1c \hat{\ve V}_1 \times \ve B_0 + \frac 1c \hat{\ve V}_0 \times \ve B_1)
\end{align}
