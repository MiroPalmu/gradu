...
but has its roots in work by Buneman \citep{buneman_dissipation_1959}
and Dawson \citep{dawson_one-dimensional_1962} in the late 1950s.
Later, two classic PIC text books were published:
one by Hockney and Eastwood \citep{hockney_computer_1981} in 1981
and another by Birdsall and Langdon \citep{langdon_plasma_1991} in 1991.
...
Maxwell's equations (see Equation~\eqref{eq:maxwells}):
\begin{align}
...
at the Yee mesh vertex locations (see Equation~\eqref{eq:yee-vertex-locations}),
...
defined on the grid vertices (see Equation~\eqref{eq:yee-vertex-locations}),
...
and $S'$ is the weighting function, normalized such that at every position\footnote{
  This normalization follows the convention used in~\citep{langdon_plasma_1991},
...
In this section, we compare properties of the relativistic Boris pusher
(1960)~\citep{boris_relativistic_1960},
Vay's method (2008)~\citep{vay_simulation_2008}, and
Higuera-Cary method (2017)~\citep{higuera_structure-preserving_2017}.
...
which is derived by taking the time derivative of Gauss’s law (see Equation~\eqref{eq:gauss-law}):
\begin{align}
...
as described in~\citep{blinne_new_2003},
...
Esirkepov~\citep{esirkepov_exact_2001}.
A general treatment of charge-conserving methods using finite-element techniques is provided by
Eastwood in~\citep{eastwood_virtual_1991},
with an extension to curvilinear coordinates in~\citep{eastwood_body-fitted_1995}.
...
This observation is consistent with the findings reported in~\citep{steiniger_ez:_2023}.
...
Thus, the continuity equation for charge density (see Equation~\eqref{eq:current-density-continuation})
...
can be computed without conditionals by using the algorithm described in~\citep{blinne_new_2003}:
\begin{align}
...
Compared to the original scheme described in~\citep{villasenor_rigorous_1992},
