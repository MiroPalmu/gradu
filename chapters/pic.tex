...
but has its roots in work by \citet{buneman_dissipation_1959}
and \citet{dawson_one-dimensional_1962}.
Later, two classic PIC text books were published:
one by \citet{hockney_computer_1981}
and another by \citet{langdon_plasma_1991}.

Basic PIC consists of many smaller tasks, for which numerous algorithms exist with varying trade-offs.
At high-evel a single PIC time step contains four parts:
field propagation, field interpolation, particle update/push, and current deposition.
In this chapter, we describe these tasks in more detail
...
with spatial component of a four-velocity $\ve u(\ve v) = \gamma \ve v$,
coordinate velocity $\ve v$, mass $m$, and charge $q$
are~\citep{verboncoeur_particle_2005,vay_modeling_2014}:
\begin{align}
...
where
\begin{align}
  \gamma = \sqrt{\frac{1}{1 - \frac{\ve v^2}{c^2}}} = \sqrt{1 + \frac{\ve u^2}{c^2}}
\end{align}
is the Lorentz factor, and the force $\ve F$ is given by the Lorentz force.

Electric and magnetic fields, $\ve E$ and $\ve B$, are evolved according to
Maxwell's equations (see Equation~\ref{eq:maxwells}):
\begin{align}
...
and the initial conditions satisfy Equations~\eqref{eq:gauss-law}
and~\eqref{eq:gauss-law-for-B},
then they are automatically satisfied throughout the rest of the simulation as
well~\citep{eastwood_virtual_1991}.
...
discretizations on a Yee mesh~\citep{yee_numerical_1966}.
In this approach,
...
\end{align}
where $\Delta x_i$ is the grid spacing in the direction of  $\hat{\ve e}_i$,
$\hat{\ve e}_i$ is the orthogonal unit vector, and $m,n,l \in \mathbb{N}$.
...
\begin{align}
  ...
  \ve x^{B_i}_{m,n,l} =
\ve x_{m,n,l} + \frac12 \sum_{r\in\{1, 2, 3\}/{i}}\Delta x_{r} \hat{\ve e}_{r} =
  \begin{cases}
  ...
  \end{cases}\,.
\end{align}
See Figure~\ref{fig:yee-mesh} for a schematic illustration of the Yee mesh.

The most common way to solve Equations~\eqref{eq:maxwell-faraday}
and~\eqref{eq:ampere-maxwell-law}
numerically is the explicit leapfrog algorithm~\citep{verboncoeur_particle_2005,langdon_plasma_1991}.
...
centered finite-difference temporal and spatial derivatives as given by Equation~\eqref{eq:centered-fd}.
...
and by construction the Yee mesh provides exactly the components we need.

\subsection{Finite-difference time domain}

We now apply the finite-difference derivatives from Equation~\eqref{eq:yee-temporal-derivative}
...
\begin{align}
  ...
  \end{bmatrix}\,,
\end{align}
where the symbols $\alpha$, $\beta$, $\gamma$, $\mu$, $\nu$, and $\rho$
are placeholders for electric or magnetic field components.
Now we apply the second-order accurate centered finite-difference
derivatives from Equations~\eqref{eq:yee-temporal-derivative} and~\eqref{eq:yee-spatial-derivative}
...
In the Runko Git repository, implementation of the second-order accurate FDTD
scheme is located at \texttt{core/emf/yee\_lattice\_FDTD2.c++}.
...
Currently the GPU port of Runko uses linear interpolation
because it is fast and simple to implement.
In the Runko Git repository, implmementation is located at
\texttt{core/emf/yee\_lattice\_interpolate\_linear\_1st.c++}.
More interpolators can be implemented later when desired.

The interpolation is performed for the fields $\ve E$ and $\ve B$, which are located
on the Yee mesh according to Equations~\eqref{eq:yee-E-locations} and~\eqref{eq:yee-B-locations}.
While interpolation could be done directly from those locations to the particle locations,
it is common practice to first compute $\ve E$ and $\ve B$
at the Yee mesh vertex locations (see Equation~\ref{eq:yee-vertex-locations}),
for example via a simple spatial averaging.
This approach simplifies the definition and implementation of interpolators
and helps reduce short-wavelength noise~\citep{langdon_plasma_1991}.

We can now express the interpolation of a generic function $f$,
defined on the grid vertices (see Equation~\ref{eq:yee-vertex-locations}),
to an arbitrary point $\ve x$:
\begin{align}
  f(\ve x) =
  \Delta V \sum_{n,m,l} f_{n,m,l} S'\left(\ve x_{n, m, l}, \ve x\right)\,,
\end{align}
where $\Delta V = \prod_i \Delta x_i$, $f_{n,m,l} \equiv f(\ve x_{n,m,l})$,
and $S'$ is the weighting function, normalized such that at every position\footnote{
  This normalization follows the convention used in~\citet{langdon_plasma_1991},
...
Starting from the zeroth-order B-spline, one can construct B-splines with arbitrarily high order,
...
\begin{figure}[h]
  ...
  \caption{Zeroth- (violet curve), first- (blue), second- (green),
    and third-order (orange) B-spline weighing functions $S$.
...
Applying the second-order accurate centered finite-difference from Equation~\eqref{eq:centered-fd}
...
(i.e., an equation that can not be solved directly but requires iterative or
other more complex algorithms).
However, various pushers have procedures that allow solving Equation~\eqref{eq:particle-update-vel}
in steps, keeping the method explicit.

In this section, we compare properties of
the relativistic Boris pusher~\citep{boris_relativistic_1970},
Vay's method~\citep{vay_simulation_2008}, and
Higuera-Cary method~\citep{higuera_structure-preserving_2017}.
Since the GPU port of Runko uses the relativistic Boris pusher,
we will examine it in more detail.
In the Runko Git repository, implementation of the Boris pusher
is located at \texttt{core/pic/particle\_boris.c++}.
From now on we will refer the relativistic Boris pusher as just the Boris pusher for short.

\subsection{Properties}

We begin by listing the choises for $\bar{\ve v}^s$.
The relativistic Boris pusher uses:
\begin{align}
  \label{eq:boris-avg-vel}
  \bar{\ve v}^s_b \equiv \frac{\ve v\left(\ve u^{s+\frac12} - \boldsymbol{\epsilon}^{s}\right)
  + \ve v\left(\ve u^{s-\frac12} + \boldsymbol{\epsilon}^{s}\right)}{2\gamma^s}\,,
\end{align}
...
It is assumed that the vectors $\ve u^{s\pm\frac12} \mp \boldsymbol{\epsilon}^{s}$
...
the solutions lie on a symplectic manifold in phase space~\citep{goldstein_classical_2014}.
...
preserves physical accuracy~\citep{higuera_structure-preserving_2017}\footnote{
...
in arbitrary electromagnetic fields.
...
whereas Vay’s and Higuera-Cary methods exhibit similar computational cost.

\subsection{Boris pusher}

We begin by substituting $\bar{\ve v}_b^s$ from Equation~\eqref{eq:boris-avg-vel}
into Equation~\eqref{eq:particle-update-vel}
...
Note that the same Lorentz factor is shared between $\ve u^f$ and $\ve u^i$
because we assume they have the same length.
We want to find a way to evaluate Equation~\eqref{eq:boris-derivation-1}
...
By taking the dot product of Equation~\eqref{eq:boris-derivation-2} with $\ve u'_+ + \ve u'_-$
...
in a different form than in Equation~\eqref{eq:boris-derivation-2}.
...
Thus, using the cyclotron frequency from Equation~\eqref{eq:cyclotron-frequency}:
\begin{align}
...
if this deposition is charge-conserving
and Equations~\eqref{eq:gauss-law} and~\eqref{eq:gauss-law-for-B}
...
conserving~\citep{hockney_computer_1981,langdon_plasma_1991,blinne_new_2003}.
For a current deposition method to be charge-conserving,
it must satisfy the continuity equation for charge density,
which is derived by taking the time derivative of Gauss’s law (see Equation~\ref{eq:gauss-law}):
\begin{align}
...
as described in~\citet{blinne_new_2003},
so this section focuses on that method and the Villasenor-Buneman method~\citep{villasenor_rigorous_1992},
since the ZigZag method can be considered an optimized version of the latter.
In the Runko Git repository, implementation of the ZigZag method
is located at \texttt{core/pic/particle\_current\_zigzag\_1st.c++}.

Another notable scheme is the density decomposition method proposed
by~\citet{esirkepov_exact_2001}.
A general treatment of charge-conserving methods using finite-element techniques
is provided by~\citet{eastwood_virtual_1991},
with an extension to curvilinear coordinates in~\citet{eastwood_body-fitted_1995}.
...
The way Villasenor-Buneman method calculates the current from a particle trajectory
can only be applied if the trajectory
...
This makes it possible to implement the ZigZag method without conditionals,
which is important for good performance on parallel hardware (e.g. GPUs).

We note that the 3D ZigZag method described in the original paper is defective
and not charge-conserving.
For particle motion confined to a single cell,
the method adds current in the same way as the Villasenor-Buneman method
(equations (35-37) in~\citep{villasenor_rigorous_1992}),
...
As a result, current deposition from diagonal trajectories is not charge-conserving.
This observation is consistent with the findings reported in~\citet{steiniger_ez:_2023}.
...
it generates current through exactly twelve faces of this staggered grid.
These twelve faces, along with the Yee mesh cell,
...
Thus, the continuity equation for charge density (see Equation~\ref{eq:current-density-continuation})
...
\begin{align}
  ...
  r^s_i &\equiv \left(\ve x^s - \ve x_{m,n,l}\right)_i
\end{align}
and let:
\begin{align}
...
and~\eqref{eq:total-current-flux-2} by cyclic permutation of the indices.
...
can be computed without conditionals by using the algorithm described in~\citet{blinne_new_2003}:
\begin{align}
...
Compared to the original scheme described in~\citet{villasenor_rigorous_1992},
the conditionless ZigZag achieves superior computational performance.
